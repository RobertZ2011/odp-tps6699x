...
use crate::command::ReturnValue;
use crate::command::vdms::{INITIATOR_WAIT_TIME_MS, MAX_NUM_DATA_OBJECTS, Version};
use crate::fw_update::UpdateConfig as FwUpdateConfig;
use crate::registers::port_config::TypeCStateMachine;
use crate::registers::{PdCcPullUp, PpExtVbusSw, PpIntVbusSw};
use crate::{MAX_SUPPORTED_PORTS, PORT0, PORT1, TPS66993_NUM_PORTS, TPS66994_NUM_PORTS};

type Updater<'a, M, B> = BorrowedUpdaterInProgress<tps6699x_drv::Tps6699x<'a, M, B>>;

/// Firmware update state
pub(super) struct FwUpdateState<'a, M: RawMutex, B: I2c> {
    /// Updater state
    pub(super) updater: Updater<'a, M, B>,
    /// Interrupt guards to maintain during the update
    ///
    /// This value is never read, only used to keep the interrupt guard alive
    #[allow(dead_code)]
    pub(super) guards: [Option<tps6699x_drv::interrupt::InterruptGuard<'a, M, B>>; MAX_SUPPORTED_PORTS],
}
...
pub enum UsbControlMethod {
    /// Set USB capabilities through [`crate::registers::tx_identity`].
    #[default]
    TxIdentity,
...
pub struct Tps6699x<'a, M: RawMutex, B: I2c> {
    pub(super) tps6699x: tps6699x_drv::Tps6699x<'a, M, B>,
    pub(super) update_state: Option<FwUpdateState<'a, M, B>>,
    /// Firmware update configuration
    pub(super) fw_update_config: FwUpdateConfig,
...
impl<'a, M: RawMutex, B: I2c> Tps6699x<'a, M, B> {
    ...
    pub(super) fn log_error(&self, e: Error<B::Error>) -> PdError {
        match e {
            Error::Bus(_) => {
    ...
    pub(super) fn guard_no_fw_update_active(&self) -> Result<(), PdError> {
...
impl<M: RawMutex, B: I2c> Pd for Tps6699x<'_, M, B> {
    ...
    async fn send_vdm(&mut self, port: LocalPortId, tx_vdm: SendVdm) -> Result<(), PdError> {
        self.guard_no_fw_update_active()?;
        let input = {
            let mut input = crate::command::vdms::Input::default();
    ...
    async fn set_usb_control(&mut self, port: LocalPortId, config: UsbControlConfig) -> Result<(), PdError> {
        ...
        match self.config.usb_control_method {
            ...
            UsbControlMethod::DpConfig => {
                use crate::registers::DpUsbDataPath;
            ...
            UsbControlMethod::TbtConfig => {
                use crate::registers::TbtUsbDataPath;
...
impl<M: RawMutex, B: I2c> Retimer for Tps6699x<'_, M, B> {
    ...
    async fn reconfigure_retimer(&mut self, port: LocalPortId) -> Result<(), PdError> {
        self.guard_no_fw_update_active()?;
        let input = {
            let mut input = crate::command::muxr::Input(0);
...
impl<M: RawMutex, B: I2c> type_c_interface::controller::power::SystemPowerStateStatus for Tps6699x<'_, M, B> {
    async fn set_system_power_state_status(
        ...
        self.guard_no_fw_update_active()?;
        use crate::registers::SystemPowerState as DriverSystemPowerState;
